...
cbind(group1 = k5_1_avg, group2 = k5_2_avg, USA = k5_3_avg, BrazilIndiaRussia = k5_4_avg, China = k5_5_avg, year = 1961:2023) %>%
  as.data.frame() %>%
  gather(key = "cluster", value = "temp", -year) %>%
  ggplot(.) +
  geom_line(aes(x=year, y=temp, color = cluster), alpha = .8)



cbind(group1 = pred_k51_long, group2 = pred_k52_long, USA = pred_k53_long, BrazilIndiaRussia = pred_k54_long, China = pred_k55_long, year = 2024:2043, total = pred_k5_long) %>%
  as.data.frame() %>%
  gather(key = "cluster", value = "temp", 1:5) %>%
  ggplot(.) +
  geom_line(aes(x=year, y=temp, color = cluster), alpha = .5) +
  geom_line(aes(x=year, y=total), color = "black") +
  ggtitle("20 Year Future Prediction")
...
cbind(group1 = hi3_1_avg, group2 = hi3_2_avg, group3 = hi3_3_avg, year = 1961:2023, world) %>%
  as.data.frame() %>%
  gather(key = "cluster", value = "temp", 1:3) %>%
  ggplot(.) +
  geom_line(aes(x=year, y=temp, color = cluster), alpha = .7) +
  geom_line(aes(x=year, y=world), color = "black")

cbind(group1 = pred_H31_long, group2 = pred_H32_long, group3 = pred_H33_long, year = 2024:2043, total = pred_H3_long) %>%
  as.data.frame() %>%
  gather(key = "cluster", value = "temp", 1:3) %>%
  ggplot(.) +
  geom_line(aes(x=year, y=temp, color = cluster), alpha = .7) +
